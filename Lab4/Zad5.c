...
int calculatePostfixFromFile(Position head, cahr* fileName, double* result);
int readFile(char* fileName, char* buffer);
int parseStringIntoPostfix(Position head, cahr* buffer, double* result);
...
int popPerformOperation(Position head, char opeartion, double* result);
...
int parseStringIntoPostfix(Position head, cahr* buffer, double* result)
{
	...
	while (strlen(currentBuffer) > 0) {
		...
		if (status != 1) {
			...
			status = popPerformOperation(head, operation, result);
...
int pop(Position head, double* result) 
{
	...
	Position poppedElement = head->next;
	*result = poppedElement->number;
